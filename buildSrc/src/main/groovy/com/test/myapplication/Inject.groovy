...
class Inject {
        ...
        if (dir.isDirectory()) {
            dir.eachFileRecurse { File file ->
                ...
                if (filePath.endsWith(".class")
                    ...
                    println '==============1' + file.name + filePath
                    // 这里是应用包名，也能从清单文件中获取，先写死
                    int index = filePath.indexOf("com/example/aidl/myapplicationttt")  // 获取包名下的.class
                    if (index != -1) {
                        int end = filePath.length() - 6 // .class = 6
                        String className = filePath.substring(index, end).replace('\\', '.').replace('/', '.')
                        println '==============2 inject class' + className
    ...
    private static void injectClass(String className, String path) {
        ...
        if (cts == null || cts.length == 0) {
            ...
            cts[0].insertBeforeBody("System.out.println(com.example.aidl.hack.AntilazyLoad.class);")
        }
        c.writeFile(path)
        c.detach()
    }
    ...
    private static void insertNewConstructor(CtClass c) {
        ...
        constructor.insertBeforeBody("System.out.println(com.example.aidl.hack.AntilazyLoad.class);")
